...
<head>
    ...
    <title>Modifica Account WhatsApp</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.1/font/bootstrap-icons.css">
</head>
<body>
<div class="container py-5">
    <h1 class="mb-3">Modifica Account WhatsApp: <strong>{{ $account->name }}</strong></h1>
    ...
    <p class="lead mb-4">Aggiorna i dettagli del tuo account. Lascia il campo "Nuovo Access Token" vuoto se non vuoi modificarlo.</p>
    ...
    <form action="{{ route('whatsapp-accounts.update', ['whatsapp_account' => $account, 'token' => $token]) }}" method="POST" class="card card-body">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <input type="text" id="business_name" name="business_name" class="form-control" value="{{ old('business_name', $account->business_name) }}" required>
            <div class="form-text">Questo è il nome visualizzato che i tuoi contatti vedranno. Se hai appena registrato il numero, potrebbe essere in attesa di approvazione da parte di Meta. Controlla lo stato di approvazione nel WhatsApp Manager: deve essere "Approvato". <strong>Non potrai inviare messaggi finché il nome non sarà approvato.</strong></div>
        </div>

        <div class="mb-3">
            <label for="phone_number_display" class="form-label">Numero di Telefono (visualizzato)</label>
            <input type="text" id="phone_number_display" name="phone_number_display" class="form-control" value="{{ old('phone_number_display', $account->phone_number_display) }}" required>
        </div>

        <div class="mb-3">
            <label for="waba_id" class="form-label">WABA ID</label>
            <input type="text" id="waba_id" name="waba_id" class="form-control" value="{{ old('waba_id', $account->waba_id) }}" required>
        </div>

        <div class="mb-3">
            <label for="phone_number_id" class="form-label">Phone Number ID</label>
            <input type="text" id="phone_number_id" name="phone_number_id" class="form-control" value="{{ old('phone_number_id', $account->phone_number_id) }}" required>
        ...
        <div class="mb-4">
            <label for="access_token" class="form-label">Nuovo Access Token</label>
            <input type="password" id="access_token" name="access_token" class="form-control" placeholder="Compila solo per aggiornare il token">
            <div class="form-text">
                Il token esistente non viene mostrato per motivi di sicurezza. Per un uso stabile, si consiglia di generare un <strong>token di accesso utente di sistema permanente</strong> dal proprio Meta Business Manager.
